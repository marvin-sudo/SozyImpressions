...
const fileHeader = `export interface BestsellerProduct {
  id: string;
  name: string;
  badge: string;
  category: string;
...
const fullContent = fileHeader + JSON.stringify(products, null, 2) + ';\n';

fs.writeFileSync('src/data/bestsellersData.ts', fullContent, 'utf8');
console.log('Successfully wrote src/data/bestsellersData.ts with', products.length, 'products');
